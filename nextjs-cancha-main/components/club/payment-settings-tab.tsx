...
export function PaymentSettingsTab() {
  ...
  // Calcular si hay cambios no guardados
  const hasUnsavedChanges = config ? (
    aceptaMercadopago !== (config.aceptaMercadopago ?? false) ||
    whatsapp !== (config.whatsapp || "") ||
    yapeNumero !== (config.yapeNumero || "") ||
    yapeQrUrl !== (config.yapeQrUrl || "") ||
    yapeTitular !== (config.yapeTitular || "") ||
    plinNumero !== (config.plinNumero || "") ||
    plinQrUrl !== ((config as any).plinQrUrl || "") ||
    plinTitular !== (config.plinTitular || "") ||
    porcentajeAdelantoDefault !== (config.porcentajeAdelantoDefault ?? 50) ||
    adelantoMinimo !== (config.adelantoMinimo ? String(config.adelantoMinimo) : "")
  ) : false;
